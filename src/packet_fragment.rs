...
pub(crate) fn clamp_first_fragment_payload_bytes(requested: usize) -> usize {
    requested.clamp(MIN_FIRST_FRAGMENT_PAYLOAD_BYTES, MAX_FIRST_FRAGMENT_PAYLOAD_BYTES)
}

pub(crate) fn openauto_continuation_fragment_payload_bytes(first_fragment_payload_bytes: usize) -> usize {
...
pub(crate) fn fragment_plain_payload(payload: &[u8], opts: PlainPayloadFragmentOptions) -> Vec<Packet> {
...
mod tests {
    ...
    fn fragment_plain_payload_preserves_non_frame_flags_without_forcing_encryption() {
        ...
        assert_eq!(packets[0].flags & FRAME_TYPE_MASK, FRAME_TYPE_FIRST | FRAME_TYPE_LAST);
