...
fn service_kind_matches_filter(filter: &str, kind: PacketDebugServiceKind) -> bool {
    ...
    split_filter_tokens(filter).any(|token| {
        token == wanted
            ...
            || match kind {
                PacketDebugServiceKind::Control => matches!(token.as_str(), "control_channel" | "control"),
                PacketDebugServiceKind::SensorSource => matches!(token.as_str(), "sensor" | "sensor_source_channel"),
                PacketDebugServiceKind::MediaSink => matches!(token.as_str(), "sink" | "media_sink_service" | "media_sink_service_channel"),
                PacketDebugServiceKind::InputSource => matches!(token.as_str(), "input" | "input_source_service" | "input_source_service_channel"),
                PacketDebugServiceKind::MediaSource => matches!(token.as_str(), "source" | "media_source_service" | "media_source_service_channel"),
                PacketDebugServiceKind::Bluetooth => matches!(token.as_str(), "bt" | "bluetooth_service" | "bluetooth_service_channel"),
                PacketDebugServiceKind::Radio => matches!(token.as_str(), "radio_service" | "radio_service_channel"),
                PacketDebugServiceKind::NavigationStatus => matches!(token.as_str(), "nav" | "navigation" | "navigation_status_service" | "navigation_status_service_channel"),
                PacketDebugServiceKind::MediaPlaybackStatus => matches!(token.as_str(), "media_playback" | "media_playback_service" | "media_playback_status_service_channel"),
                PacketDebugServiceKind::PhoneStatus => matches!(token.as_str(), "phone" | "phone_status_service" | "phone_status_service_channel"),
                PacketDebugServiceKind::MediaBrowser => matches!(token.as_str(), "browser" | "media_browser_service" | "media_browser_service_channel"),
                PacketDebugServiceKind::VendorExtension => matches!(token.as_str(), "vendor" | "vec" | "vendor_extension_service" | "vendor_extension_service_channel"),
                PacketDebugServiceKind::GenericNotification => matches!(token.as_str(), "notification" | "generic_notification_service" | "generic_notification_service_channel"),
                PacketDebugServiceKind::WifiProjection => matches!(token.as_str(), "wifi" | "wifi_projection_service" | "wifi_projection_service_channel"),
                PacketDebugServiceKind::CarProperty => matches!(token.as_str(), "car_property_service" | "car_property_service_channel"),
...
fn format_packet_for_debug(pkt: &Packet, max_payload_bytes: Option<usize>) -> String {
    ...
    if truncated {
        out.push_str(&format!("\n ... truncated {} byte(s)", payload_len - shown_len));
    }
    out
}
...
fn bytes_preview(data: &[u8], max_bytes: usize) -> String {
    ...
    if shown_len < data.len() {
        out.push_str(&format!(" ... truncated {} byte(s)", data.len() - shown_len));
    }
    out
}
...
fn json_payload_len_and_preview(value: &serde_json::Value, name: &str) -> Option<String> {
    ...
    if let Some(s) = raw.as_str() {
        return Some(format!("{} chars, preview={}", s.len(), trim_preview(s, 240)));
    }
...
fn message_name_for_kind(
    ...
    match service_kind {
        PacketDebugServiceKind::Control => format!("{:?}", ControlMessageType::from_i32(message_id.into())),
        PacketDebugServiceKind::SensorSource => format!("{:?}", SensorMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::MediaSink | PacketDebugServiceKind::MediaSource => {
            format!("{:?}", MediaMessageId::from_i32(message_id.into()))
        }
        PacketDebugServiceKind::InputSource => format!("{:?}", InputMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::Bluetooth => format!("{:?}", BluetoothMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::WifiProjection => format!("{:?}", WifiProjectionMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::Radio => format!("{:?}", RadioMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::NavigationStatus => {
            format!("{:?}", NavigationStatusMessageId::from_i32(message_id.into()))
        }
        PacketDebugServiceKind::MediaPlaybackStatus => {
            format!("{:?}", MediaPlaybackStatusMessageId::from_i32(message_id.into()))
        }
        PacketDebugServiceKind::PhoneStatus => format!("{:?}", PhoneStatusMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::MediaBrowser => format!("{:?}", MediaBrowserMessageId::from_i32(message_id.into())),
        PacketDebugServiceKind::GenericNotification => {
            format!("{:?}", GenericNotificationMessageId::from_i32(message_id.into()))
        }
        PacketDebugServiceKind::VendorExtension => {
...
fn pretty_control_message(control: Option<ControlMessageType>, data: &[u8]) -> Option<String> {
    match control.unwrap_or(MESSAGE_UNEXPECTED_MESSAGE) {
        ...
        MESSAGE_CAR_CONNECTED_DEVICES_REQUEST => parse_pretty_message!(CarConnectedDevicesRequest, data),
        ...
        MESSAGE_BATTERY_STATUS_NOTIFICATION => parse_pretty_message!(BatteryStatusNotification, data),
...
fn pretty_sensor_message(message_id: u16, data: &[u8]) -> Option<String> {
    match SensorMessageId::from_i32(message_id.into()).unwrap_or(SensorMessageId::SENSOR_MESSAGE_ERROR) {
        ...
        SensorMessageId::SENSOR_MESSAGE_ERROR => Some(format!("SENSOR_MESSAGE_ERROR raw_len={}", data.len())),
    }
}
...
fn pretty_media_message(message_id: u16, data: &[u8]) -> Option<String> {
    match MediaMessageId::from_i32(message_id.into()).unwrap_or(MediaMessageId::MEDIA_MESSAGE_DATA) {
        MediaMessageId::MEDIA_MESSAGE_DATA => Some(format!("MEDIA_MESSAGE_DATA {}", bytes_preview(data, 64))),
        MediaMessageId::MEDIA_MESSAGE_CODEC_CONFIG => Some(format!("MEDIA_MESSAGE_CODEC_CONFIG {}", bytes_preview(data, 64))),
        ...
        MediaMessageId::MEDIA_MESSAGE_MICROPHONE_REQUEST => parse_pretty_message!(MicrophoneRequest, data),
        MediaMessageId::MEDIA_MESSAGE_MICROPHONE_RESPONSE => parse_pretty_message!(MicrophoneResponse, data),
        MediaMessageId::MEDIA_MESSAGE_VIDEO_FOCUS_REQUEST => parse_pretty_message!(VideoFocusRequestNotification, data),
        MediaMessageId::MEDIA_MESSAGE_VIDEO_FOCUS_NOTIFICATION => parse_pretty_message!(VideoFocusNotification, data),
        MediaMessageId::MEDIA_MESSAGE_UPDATE_UI_CONFIG_REQUEST => parse_pretty_message!(UpdateUiConfigRequest, data),
        MediaMessageId::MEDIA_MESSAGE_UPDATE_UI_CONFIG_REPLY => parse_pretty_message!(UpdateUiConfigReply, data),
        MediaMessageId::MEDIA_MESSAGE_AUDIO_UNDERFLOW_NOTIFICATION => parse_pretty_message!(AudioUnderflowNotification, data),
    }
}
...
fn pretty_input_message(message_id: u16, data: &[u8]) -> Option<String> {
    match InputMessageId::from_i32(message_id.into())? {
        ...
        InputMessageId::INPUT_MESSAGE_KEY_BINDING_REQUEST => parse_pretty_message!(KeyBindingRequest, data),
        InputMessageId::INPUT_MESSAGE_KEY_BINDING_RESPONSE => parse_pretty_message!(KeyBindingResponse, data),
...
fn pretty_bluetooth_message(message_id: u16, data: &[u8]) -> Option<String> {
    match BluetoothMessageId::from_i32(message_id.into())? {
        BluetoothMessageId::BLUETOOTH_MESSAGE_PAIRING_REQUEST => parse_pretty_message!(BluetoothPairingRequest, data),
        BluetoothMessageId::BLUETOOTH_MESSAGE_PAIRING_RESPONSE => parse_pretty_message!(BluetoothPairingResponse, data),
        BluetoothMessageId::BLUETOOTH_MESSAGE_AUTHENTICATION_DATA => parse_pretty_message!(BluetoothAuthenticationData, data),
        BluetoothMessageId::BLUETOOTH_MESSAGE_AUTHENTICATION_RESULT => parse_pretty_message!(BluetoothAuthenticationResult, data),
    }
}

fn pretty_wifi_message(message_id: u16, data: &[u8]) -> Option<String> {
    match WifiProjectionMessageId::from_i32(message_id.into())? {
        WifiProjectionMessageId::WIFI_MESSAGE_CREDENTIALS_REQUEST => parse_pretty_message!(WifiCredentialsRequest, data),
        WifiProjectionMessageId::WIFI_MESSAGE_CREDENTIALS_RESPONSE => parse_pretty_message!(WifiCredentialsResponse, data),
    }
}

fn pretty_radio_message(message_id: u16, data: &[u8]) -> Option<String> {
    match RadioMessageId::from_i32(message_id.into())? {
        RadioMessageId::RADIO_MESSAGE_ACTIVE_RADIO_NOTIFICATION => parse_pretty_message!(ActiveRadioNotification, data),
        RadioMessageId::RADIO_MESSAGE_SELECT_ACTIVE_RADIO_REQUEST => parse_pretty_message!(SelectActiveRadioRequest, data),
        RadioMessageId::RADIO_MESSAGE_STEP_CHANNEL_REQUEST => parse_pretty_message!(StepChannelRequest, data),
        RadioMessageId::RADIO_MESSAGE_STEP_CHANNEL_RESPONSE => parse_pretty_message!(StepChannelResponse, data),
        RadioMessageId::RADIO_MESSAGE_SEEK_STATION_REQUEST => parse_pretty_message!(SeekStationRequest, data),
        RadioMessageId::RADIO_MESSAGE_SEEK_STATION_RESPONSE => parse_pretty_message!(SeekStationResponse, data),
        RadioMessageId::RADIO_MESSAGE_SCAN_STATIONS_REQUEST => parse_pretty_message!(ScanStationsRequest, data),
        RadioMessageId::RADIO_MESSAGE_SCAN_STATIONS_RESPONSE => parse_pretty_message!(ScanStationsResponse, data),
        RadioMessageId::RADIO_MESSAGE_TUNE_TO_STATION_REQUEST => parse_pretty_message!(TuneToStationRequest, data),
        RadioMessageId::RADIO_MESSAGE_TUNE_TO_STATION_RESPONSE => parse_pretty_message!(TuneToStationResponse, data),
        RadioMessageId::RADIO_MESSAGE_GET_PROGRAM_LIST_REQUEST => parse_pretty_message!(GetProgramListRequest, data),
        RadioMessageId::RADIO_MESSAGE_GET_PROGRAM_LIST_RESPONSE => parse_pretty_message!(GetProgramListResponse, data),
        RadioMessageId::RADIO_MESSAGE_STATION_PRESETS_NOTIFICATION => parse_pretty_message!(StationPresetsNotification, data),
        RadioMessageId::RADIO_MESSAGE_CANCEL_OPERATIONS_REQUEST => parse_pretty_message!(CancelRadioOperationsRequest, data),
        RadioMessageId::RADIO_MESSAGE_CANCEL_OPERATIONS_RESPONSE => parse_pretty_message!(CancelRadioOperationsResponse, data),
        RadioMessageId::RADIO_MESSAGE_CONFIGURE_CHANNEL_SPACING_REQUEST => parse_pretty_message!(ConfigureChannelSpacingRequest, data),
        RadioMessageId::RADIO_MESSAGE_CONFIGURE_CHANNEL_SPACING_RESPONSE => parse_pretty_message!(ConfigureChannelSpacingResponse, data),
        RadioMessageId::RADIO_MESSAGE_RADIO_STATION_INFO_NOTIFICATION => parse_pretty_message!(RadioStationInfoNotification, data),
        RadioMessageId::RADIO_MESSAGE_MUTE_RADIO_REQUEST => parse_pretty_message!(MuteRadioRequest, data),
        RadioMessageId::RADIO_MESSAGE_MUTE_RADIO_RESPONSE => parse_pretty_message!(MuteRadioResponse, data),
        RadioMessageId::RADIO_MESSAGE_GET_TRAFFIC_UPDATE_REQUEST => parse_pretty_message!(GetTrafficUpdateRequest, data),
        RadioMessageId::RADIO_MESSAGE_GET_TRAFFIC_UPDATE_RESPONSE => parse_pretty_message!(GetTrafficUpdateResponse, data),
        RadioMessageId::RADIO_MESSAGE_RADIO_SOURCE_REQUEST => parse_pretty_message!(RadioSourceRequest, data),
        RadioMessageId::RADIO_MESSAGE_RADIO_SOURCE_RESPONSE => parse_pretty_message!(RadioSourceResponse, data),
        RadioMessageId::RADIO_MESSAGE_STATE_NOTIFICATION => parse_pretty_message!(RadioStateNotification, data),
    }
}
...
fn pretty_navigation_status_message(message_id: u16, data: &[u8]) -> Option<String> {
    match NavigationStatusMessageId::from_i32(message_id.into())? {
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_START => parse_pretty_message!(NavigationStatusStart, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_STOP => parse_pretty_message!(NavigationStatusStop, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_NAVIGATION_STATUS => parse_pretty_message!(NavigationStatus, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_NAVIGATION_TURN_EVENT => parse_pretty_message!(NavigationNextTurnEvent, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_NAVIGATION_DISTANCE_EVENT => parse_pretty_message!(NavigationNextTurnDistanceEvent, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_NAVIGATION_STATE => parse_pretty_message!(NavigationState, data),
        NavigationStatusMessageId::INSTRUMENT_CLUSTER_NAVIGATION_CURRENT_POSITION => parse_pretty_message!(NavigationCurrentPosition, data),
    }
}
...
fn pretty_media_playback_status_message(message_id: u16, data: &[u8]) -> Option<String> {
    match MediaPlaybackStatusMessageId::from_i32(message_id.into())? {
        MediaPlaybackStatusMessageId::MEDIA_PLAYBACK_STATUS => parse_pretty_message!(MediaPlaybackStatus, data),
        MediaPlaybackStatusMessageId::MEDIA_PLAYBACK_INPUT => parse_pretty_message!(InstrumentClusterInput, data),
        MediaPlaybackStatusMessageId::MEDIA_PLAYBACK_METADATA => parse_pretty_message!(MediaPlaybackMetadata, data),
    }
}
...
fn pretty_gal_verification_message(message_id: u16, data: &[u8]) -> Option<String> {
    match GalVerificationVendorExtensionMessageId::from_i32(message_id.into())? {
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_SET_SENSOR => parse_pretty_message!(GalVerificationSetSensor, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_MEDIA_SINK_STATUS => parse_pretty_message!(GalVerificationMediaSinkStatus, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_VIDEO_FOCUS => parse_pretty_message!(GalVerificationVideoFocus, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_AUDIO_FOCUS => parse_pretty_message!(GalVerificationAudioFocus, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_INJECT_INPUT => parse_pretty_message!(GalVerificationInjectInput, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_BUG_REPORT_REQUEST => parse_pretty_message!(GalVerificationBugReportRequest, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_BUG_REPORT_RESPONSE => parse_pretty_message!(GalVerificationBugReportResponse, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_SCREEN_CAPTURE_REQUEST => parse_pretty_message!(GalVerificationScreenCaptureRequest, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_SCREEN_CAPTURE_RESPONSE => parse_pretty_message!(GalVerificationScreenCaptureResponse, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_DISPLAY_INFORMATION_REQUEST => parse_pretty_message!(GalVerificationDisplayInformationRequest, data),
        GalVerificationVendorExtensionMessageId::GAL_VERIFICATION_DISPLAY_INFORMATION_RESPONSE => parse_pretty_message!(GalVerificationDisplayInformationResponse, data),
    }
}
...
fn pretty_generic_notification_message(message_id: u16, data: &[u8]) -> Option<String> {
    match GenericNotificationMessageId::from_i32(message_id.into())? {
        GenericNotificationMessageId::GENERIC_NOTIFICATION_SUBSCRIBE => parse_pretty_message!(GenericNotificationSubscribe, data),
        GenericNotificationMessageId::GENERIC_NOTIFICATION_UNSUBSCRIBE => parse_pretty_message!(GenericNotificationUnsubscribe, data),
        GenericNotificationMessageId::GENERIC_NOTIFICATION_MESSAGE => parse_pretty_message!(GenericNotificationMessage, data),
        GenericNotificationMessageId::GENERIC_NOTIFICATION_ACK => parse_pretty_message!(GenericNotificationAck, data),
    }
}
...
fn pretty_packet_message(
    ...
    match service_kind {
        ...
        PacketDebugServiceKind::NavigationStatus => pretty_navigation_status_message(message_id, data),
        PacketDebugServiceKind::MediaPlaybackStatus => pretty_media_playback_status_message(message_id, data),
        ...
        PacketDebugServiceKind::GenericNotification => pretty_generic_notification_message(message_id, data),
