...
pub fn set_current_phone_from_bt(mac: &str, name: Option<String>) {
    ...
    let id_source = if normalized_name.is_empty() {
        ...
        format!("{}|{}", normalized_mac, normalized_name.to_ascii_lowercase())
    };
    let identity = PhoneIdentity {
        ...
        bt_name: if normalized_name.is_empty() { None } else { Some(normalized_name) },
...
pub async fn get_vehicle_profile(path: PathBuf, vehicle_id: &str) -> Result<Option<SdrUiVehicleProfile>> {
...
pub async fn process_service_discovery_response(
    ...
    if autocreate {
        changed |= ensure_vehicle_profile(&mut profiles, &vehicle_id, &vehicle_name, &vehicle_info, &snapshot);
    ...
    if cfg.sdr_ui_override_enabled && file_enabled {
        if let Some(vehicle_profile) = profiles.vehicles.iter().find(|v| v.id == vehicle_id) {
            ...
            if let Some(phone) = &phone {
                if let Some(phone_profile) = vehicle_profile.phones.iter().find(|p| p.id == phone.id) {
                    phone_profile_exists = true;
                    phone_profile_enabled = phone_profile.enabled;
                    if vehicle_profile.enabled && phone_profile.enabled {
                        patch_count += apply_display_profiles(msg, &phone_profile.displays, "phone")?;
                    }
                }
            }
        }
    ...
    let profile_exists = profile_exists_before || profiles.vehicles.iter().any(|v| v.id == vehicle_id);
...
fn merge_missing_displays(target: &mut Vec<SdrUiDisplayProfile>, snapshot: &[SdrUiDisplayProfile]) -> bool {
    let mut changed = false;
    for display in snapshot {
        if let Some(existing_display) = target.iter_mut().find(|d| display_matches(d, display.display_id, &display.display_type)) {
            for video in &display.video_configs {
                if !existing_display.video_configs.iter().any(|v| v.codec_resolution == video.codec_resolution) {
...
fn apply_insets(
    ...
    let before = (current.top(), current.bottom(), current.left(), current.right());
    ...
    before != (current.top(), current.bottom(), current.left(), current.right())
}
...
fn keep_insets_inside_resolution(
    ...
    if insets.left().saturating_add(insets.right()) >= width {
        let allowed_right = width.saturating_sub(1).saturating_sub(insets.left().min(width.saturating_sub(1)));
        warn!(
    ...
    if insets.top().saturating_add(insets.bottom()) >= height {
        let allowed_bottom = height.saturating_sub(1).saturating_sub(insets.top().min(height.saturating_sub(1)));
        warn!(
...
fn display_matches(profile: &SdrUiDisplayProfile, display_id: u32, display_type: &str) -> bool {
    profile.display_id == display_id && (profile.display_type.is_empty() || profile.display_type == display_type)
}
