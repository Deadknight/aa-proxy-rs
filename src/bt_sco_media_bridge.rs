...
pub fn set_target_channel(
    ...
    runtime.output_limiter.store(limiter.as_u8(), Ordering::SeqCst);
    runtime.start_on_first_audio.store(start_on_first_audio, Ordering::SeqCst);
    runtime.audio_peak_threshold.store(audio_peak_threshold, Ordering::SeqCst);
    runtime.start_timeout_ms.store(start_timeout_ms.max(1), Ordering::SeqCst);
    runtime.fixed_cadence.store(fixed_cadence, Ordering::SeqCst);
    runtime.cadence_ms.store(cadence_ms.max(1), Ordering::SeqCst);
    runtime.jitter_buffer_ms.store(jitter_buffer_ms, Ordering::SeqCst);
...
fn bridge_loop(runtime: Arc<Runtime>) {
    ...
    loop {
        ...
        let limiter = BtScoMediaBridgeLimiter::from_u8(runtime.output_limiter.load(Ordering::SeqCst));
        ...
        let mut adapted = adapt_48k_stereo_s16le_to_target(&frame.pcm, target_channels, gain_percent, limiter);
...
fn ensure_media_started_for_frame(
    ...
    let timeout = Duration::from_millis(runtime.start_timeout_ms.load(Ordering::SeqCst).max(1) as u64);
...
fn soft_limit_i16(sample: i32) -> i16 {
    ...
    let compressed = THRESHOLD
        + ((headroom as i64 * excess as i64) / (excess as i64 + headroom as i64)) as i32;
