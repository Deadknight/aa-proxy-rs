use crate::bt_sco;
use crate::mitm::protos::{ChannelOpenResponse, Config as AudioConfig, MediaMessageId, Start, Stop};
...
pub fn init_or_update(tx: Sender<Packet>) {
    ...
    {
        Ok(_) => debug!("{} piggyback media bridge worker started", NAME),
        Err(e) => warn!("{} failed to start piggyback media bridge worker: {}", NAME, e),
    }
...
pub fn set_target_channel(
    ...
    runtime.target_sample_rate.store(sample_rate.max(1), Ordering::SeqCst);
    runtime.target_channels.store(channels.max(1), Ordering::SeqCst);
    runtime.target_bits.store(bits, Ordering::SeqCst);
    runtime.output_gain_percent.store(gain_percent.max(1), Ordering::SeqCst);
    runtime.start_existing.store(start_existing, Ordering::SeqCst);
    runtime.stop_existing_on_disconnect.store(stop_existing_on_disconnect, Ordering::SeqCst);
    ...
    if previous != channel {
        debug!(
            "{} selected existing AA PCM sink channel=<b>{:#04x}</>, {}Hz, {}ch, {}bit, gain={}%, start_existing={}, stop_on_disconnect={}, epoch={}; will piggyback DATA only",
            NAME, channel, sample_rate, channels, bits, gain_percent, start_existing, stop_existing_on_disconnect, epoch
        );
    } else {
        debug!(
            "{} re-armed existing AA PCM sink channel=<b>{:#04x}</>, {}Hz, {}ch, {}bit, gain={}%, start_existing={}, stop_on_disconnect={}, epoch={}; will piggyback DATA only",
            NAME, channel, sample_rate, channels, bits, gain_percent, start_existing, stop_existing_on_disconnect, epoch
        );
    }
}
...
pub fn notify_media_config(channel: u8, cfg: &AudioConfig) {
    ...
    runtime.configuration_index.store(config_index, Ordering::SeqCst);
...
fn bridge_loop(runtime: Arc<Runtime>) {
    ...
    loop {
        ...
        handle_media_start_stop_state(&runtime, channel, epoch, &mut media_started_generation, &mut pts_us, &mut data_packets);

        let Some(pcm) = bt_sco::pop_aa_pcm_frame() else {
            thread::sleep(IDLE_SLEEP);
            continue;
        };
        ...
        let mut adapted = adapt_48k_stereo_s16le_to_target(&pcm, target_channels, gain_percent);
...
fn handle_media_start_stop_state(
    runtime: &Runtime,
    channel: u8,
    epoch: u64,
    ...
    if !connected {
        if *media_started_generation != 0 {
            ...
            debug!(
                "{} media bridge stopped for ch=<b>{:#04x}</>, sco_generation={}",
                NAME, channel, *media_started_generation
            );
            *media_started_generation = 0;
            *pts_us = 0;
            *data_packets = 0;
    ...
    if generation == 0 || *media_started_generation == generation {
        return;
    }
    ...
    if runtime.start_existing.load(Ordering::SeqCst) {
        ...
        if send_media_start(runtime, channel, DEFAULT_SESSION_ID, config_index).is_ok() {
            debug!(
                "{} media bridge START sent for existing ch=<b>{:#04x}</>, session_id={}, config_index={}, sco_generation={}, epoch={}",
                NAME, channel, DEFAULT_SESSION_ID, config_index, generation, epoch
            );
        }
    } else {
        debug!(
            "{} media bridge using DATA-only mode for ch=<b>{:#04x}</>, sco_generation={}",
...
fn send_microphone_request(runtime: &Runtime, channel: u8, open: bool) -> std::result::Result<(), ()> {
    let mut payload = build_microphone_request_payload(open);
    push_media_message_id(&mut payload, MediaMessageId::MEDIA_MESSAGE_MICROPHONE_REQUEST as u16);
...
fn adapt_48k_stereo_s16le_to_target(input: &[u8], target_channels: u32, gain_percent: u32) -> Vec<u8> {
    ...
    if target_channels == 1 {
        ...
        for frame in input[..even_len].chunks_exact(4) {
            ...
            out.extend_from_slice(&apply_gain(sample, gain).to_le_bytes());
        }
        out
    } else {
...
fn apply_gain(sample: i16, gain_percent: i32) -> i16 {
...
fn drop_stale_sco_frames() {
    while bt_sco::pop_aa_pcm_frame().is_some() {}
}
